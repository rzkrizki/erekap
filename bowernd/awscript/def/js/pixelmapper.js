(function() {
    ...
    pixelMapper.createMap = function(mapFile) {
        ...
        if(mapFile!='indonesia'){
            ...
            pixelMapper.map.addListener("clickMapObject", pixelMapper.handleSubClickMapObject);
            if(AWScript.defaults.randColor == true){
                pixelMapper.map.addListener("init", pixelMapper.handleRandomColor);
            }
            pixelMapper.handleSubClickMapObjectOption();
        } else {
            pixelMapper.map.addListener("clickMapObject", pixelMapper.handleClickMapObject);
        }

        pixelMapper.handleOptions(mapFile);
    ...
    pixelMapper.setAreaTitle = function(){
        ...
        for (var x in pixelMapper.map.dataProvider.areas) {
            ...
            imagesSettings.labelFontSize = "14";
    ...
    pixelMapper.getObjectClick = function(object){
        var value = object.title.replace(/ /g, '_');
        AWScript.defaults.map = 'dp_'+value.toLowerCase()+'_ri';
        pixelMapper.generateSelect('#area_provinsi', AWScript.defaults.map+'.js', 'Pilih Area Provinsi');
        AWScript.initialize();
        // jQuery('.maps').find('input').val(value.toUpperCase());
        load_dapil('#area_dapil', pixelMapper.selectedObject.title);
    }
    ...
    pixelMapper.handleSubClickMapObject = function(event){
        var clickedObject = event.mapObject;
        var zoomLevel = pixelMapper.map.zoomLevel();
        pixelMapper.lineColor = pixelMapper.handleColor(AWScript.defaults.color);
        pixelMapper.movingObject = undefined;
        pixelMapper.map.dragMap = true;
        pixelMapper.selectedObject = clickedObject;
        AWScript.defaults.selectedMap = pixelMapper.selectedObject;
        pixelMapper.generateSelect('#area_dapil', AWScript.defaults.selectedMap.id, 'Pilih Area Dapil');
        setTimeout(function(){
            jQuery('#openModal').click();
        }, 400);
    };
    pixelMapper.handleRandomColor = function(){
        var area = pixelMapper.map.dataProvider.areas;
        var areax;

        for (i in area) {
            var areas = area[i];
            var areax = pixelMapper.map.getObjectById(areas.id);
            areas.pixelMapperArea = true;
            areax.color = coloringItems();
            areax.colorReal = coloringItems();
            areax.validate();
        }
        ...
        function coloringItems(){
            ...
            return pixelMapper.handleColor(color);
        }
    };
    pixelMapper.handleSubClickMapObjectOption = function(){
        var selectedObject = pixelMapper.map.dataProvider;
        jQuery('#area_dapil').unbind().change(function(){
            var areasSelected = selectedObject.areas.find(x => x.id === this.value);
            pixelMapper.selectedObject = areasSelected;
            AWScript.defaults.selectedMap = pixelMapper.selectedObject;
            setTimeout(function(){
                jQuery('#openModal').click();
            }, 400);
    ...
    pixelMapper.generateSelect = function(target, value, placeholder){
        if(value=='undefined' || value==null || value==''){
            jQuery(target).select2({placeholder: placeholder, width: '100%'});
        } else {
            jQuery(target).val(value).select2({placeholder: placeholder, width: '100%'});
        }
    };
    pixelMapper.handleOptions = function(object){
        if(object=='indonesia'){
            jQuery('.section-four').find(".dapil").hide();
            jQuery('.section-four').find(".nasional").removeClass('d-none').addClass('d-flex');
        } else {
            jQuery('.section-four').find(".dapil").show();
            jQuery('.section-four').find(".nasional").removeClass('d-flex').addClass('d-none');
        }
    };
})();