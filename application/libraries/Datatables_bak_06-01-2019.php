<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
  ...
  */
  class Datatables
  {
    ...
    */
// test tambah having
    public function having($column,$val)
    {
      $this->ci->db->having($column,$val, false);
      return $this;
    }
    ...
    */
    ...
    public function generate_custom($sql = '', $output = 'json', $charset = 'UTF-8')
    {
      if(strtolower($output) == 'json')
       // $this->get_paging();
      //$this->get_ordering();
      //$this->get_filtering();
      return $this->produce_output_custom($sql, strtolower($output), strtolower($charset));
    }
    /**
    ...
    */
    ...
    private function produce_output_custom($sql, $output, $charset)
    {
      $aaData = array();
      $rResult = $this->ci->db->query($sql);
      if($output == 'json')
      {
        $iTotal = $rResult->num_rows();//$this->get_total_results();
        $iFilteredTotal = $rResult->num_rows();//$this->get_total_results(TRUE);      
      }
      foreach($rResult->result_array() as $row_key => $row_val)
      {
        $aaData[$row_key] =  ($this->check_cType())? $row_val : array_values($row_val);
